...
<head>
    ...
    <meta name="description"
        content="Portal pendaftaran turnamen esports MIRAI Indonesia. Daftar dan ikuti turnamen Mobile Legends, Valorant, PUBG, dan game lainnya.">
...
<body class="bg-[#050505] text-zinc-100 overflow-x-hidden" x-data>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <div class="hidden md:flex items-center gap-8">
                    ...
                    <a href="https://mirai-id.netlify.app/" target="_blank"
                        class="text-sm text-gray-400 hover:text-cyan-400 transition">Company Profile</a>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    @else
                        ...
                        <a href="{{ route('auth.register') }}"
                            class="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-4 py-2 rounded text-sm font-bold hover:opacity-90 transition shadow-[0_0_15px_rgba(6,182,212,0.3)] cursor-pointer">Daftar</a>
                    @endauth
                </div>
            </div>
        </div>
    ...
    <!-- Hero - Full Screen -->
    <section id="home"
        class="min-h-screen flex items-center justify-center pt-20 relative overflow-hidden grid-bg">
        <!-- Glow Effect -->
        <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-cyan-500/20 rounded-full blur-[120px] pointer-events-none">
        </div>

        <div class="text-center px-4 max-w-5xl mx-auto z-10 relative">
            <div class="mb-4 inline-block">
                <span
                    class="bg-[#111] border border-cyan-500/30 text-cyan-400 text-[10px] md:text-xs font-bold px-4 py-1 rounded-full uppercase tracking-[0.3em]">Portal
                    Turnamen Esports</span>
            </div>

            <!-- Master Logo -->
            <div class="w-full max-w-xl mx-auto mb-6 transform hover:scale-105 transition duration-500">
                <svg viewBox="0 0 500 150" xmlns="http://www.w3.org/2000/svg"
                    class="w-full drop-shadow-[0_0_30px_rgba(6,182,212,0.3)]">
                    <defs>
                        <linearGradient id="heroGrad" x1="0" y1="0" x2="130" y2="0"
                    ...
                    <text x="180" y="105" font-family="'Orbitron', sans-serif" font-weight="900" font-size="80"
                        fill="white" letter-spacing="-2">IRAI</text>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="#tournaments"
                    class="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-8 py-3 rounded font-display font-bold hover:opacity-90 transition shadow-[0_0_20px_rgba(6,182,212,0.4)] cursor-pointer">Lihat
                    Tournament</a>
                <a href="https://mirai-id.netlify.app/" target="_blank"
                    class="border border-gray-600 text-white px-8 py-3 rounded-lg hover:border-cyan-500 hover:bg-cyan-500/10 transition cursor-pointer">Tentang
                    MIRAI</a>
            </div>
        </div>
    </section>
    ...
    <section id="tournaments" class="py-24 px-6 bg-black border-y border-white/5">
        <div class="max-w-6xl mx-auto">
            <div class="text-center mb-16">
                <h2 class="text-3xl md:text-4xl font-display font-bold text-white mb-2">TOURNAMENT <span
                        class="gradient-text">AKTIF</span></h2>
            ...
            @if ($tournaments->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($tournaments as $tournament)
                        <a href="{{ route('register', $tournament->slug) }}"
                            class="bg-[#111] border border-gray-800 rounded-xl overflow-hidden hover:border-cyan-500/50 transition group cursor-pointer">
                            @if ($tournament->banner_path)
                                <img src="{{ Storage::url($tournament->banner_path) }}"
                                    alt="{{ $tournament->title }}" class="w-full h-40 object-cover">
                            @else
                                <div
                                    class="w-full h-40 bg-gradient-to-br from-cyan-500/20 to-blue-600/20 flex items-center justify-center">
                                    <span
                                        class="text-4xl font-bold text-zinc-700">{{ strtoupper(substr($tournament->game_type, 0, 2)) }}</span>
                                </div>
                            @endif
                            ...
                            <div class="p-5">
                                <div class="flex items-center gap-2 mb-2">
                                    <span
                                        class="px-2 py-0.5 text-xs bg-cyan-500/10 text-cyan-400 rounded">{{ $tournament->game?->abbreviation ?? strtoupper($tournament->game_type) }}</span>
                                    @if ($tournament->fee == 0)
                                        <span
                                            class="px-2 py-0.5 text-xs bg-green-500/10 text-green-400 rounded">GRATIS</span>
                                    @endif
                                </div>
                                <h3 class="text-lg font-semibold mb-2 group-hover:text-cyan-400 transition">
                                    {{ $tournament->title }}</h3>
                                <p class="text-sm text-zinc-400 mb-3">
                                    {{ $tournament->event_date->format('d M Y, H:i') }} WIB</p>

                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-sm text-zinc-500">{{ $tournament->participants_count }}/{{ $tournament->max_slots }}
                                        slot</span>
                                    @if ($tournament->fee > 0)
                                        <span class="text-cyan-400 font-semibold">Rp
                                            {{ number_format($tournament->fee, 0, ',', '.') }}</span>
                                    @endif
                                </div>
                            </div>
                        </a>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div>
                    ...
                    <h3 class="text-3xl md:text-4xl font-display font-bold text-white mb-6">MIRAI <span
                            class="gradient-text">INDONESIA</span></h3>

                    <p class="text-gray-400 mb-6 leading-relaxed">
                        <strong class="text-white">MIRAI</strong> (ミライ) berarti "Masa Depan" dalam bahasa Jepang. Kami
                        adalah penyedia jasa Event Organizer dan Live Streaming profesional untuk turnamen esports.
                    </p>
                    <p class="text-gray-400 mb-8 leading-relaxed">
                        Dari turnamen komunitas hingga kompetisi besar, kami menghadirkan standar visual profesional
                        (MPL / VCT) ke dalam setiap event.
                    </p>

                    <a href="https://mirai-id.netlify.app/" target="_blank"
                        class="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-6 py-3 rounded font-display font-bold hover:opacity-90 transition shadow-[0_0_20px_rgba(6,182,212,0.4)] cursor-pointer">
                ...
                <div class="relative">
                    <div
                        class="absolute inset-0 bg-gradient-to-tr from-cyan-500/20 to-blue-500/20 blur-3xl rounded-full">
                    </div>
                    <div
                        class="relative bg-[#111] border border-gray-800 p-8 rounded-2xl grid grid-cols-2 gap-8 text-center">
                        <div>
    ...
    <footer class="border-t border-white/5 py-8 px-4 bg-black/50">
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-6 text-sm text-gray-500">
                <a href="https://mirai-id.netlify.app/" target="_blank"
                    class="hover:text-cyan-400 transition cursor-pointer">Company Profile</a>
                <a href="https://wa.me/6281234567890" target="_blank"
                    class="hover:text-cyan-400 transition cursor-pointer">Contact</a>
            </div>
