...
@endphp
<div class="max-w-xl mx-auto py-10 px-4 pb-32"
            ...
            @if($tournament->fee > 0)
                <div class="bg-black/30 border border-gray-800 rounded-lg p-4 text-left mb-6">
                    ...
                    <ol class="list-decimal list-inside space-y-2 text-gray-400 text-sm">
                        <li>Lakukan pembayaran sebesar <span class="text-cyan-400 font-semibold">Rp {{ number_format($tournament->fee, 0, ',', '.') }}</span></li>
    ...
    @else
        ...
        @else
            ...
            <div class="bg-[#1a1a1a] border-t-8 border-cyan-500 rounded-xl p-6 mb-6 shadow-2xl">
                ...
                <p class="text-gray-400 text-sm mb-4 whitespace-pre-wrap">{{ $tournament->description ?: 'Silakan isi data dengan benar.' }}</p>
                <div class="flex flex-wrap gap-4 text-xs border-t border-gray-800 pt-4">
                    <span class="flex items-center gap-1 text-cyan-400">
                        ...
                        {{ \App\Models\UserGameProfile::getGameName($tournament->game_type) }}
                    </span>
                    <span class="flex items-center gap-1 text-cyan-400">
            ...
            <div class="mb-6 flex items-center gap-4">
                <div class="flex-1 bg-gray-800 rounded-full h-2 overflow-hidden">
                    ...
                    <div class="h-full bg-gradient-to-r from-cyan-500 to-blue-600" style="width: {{ $percentage }}%"></div>
                </div>
                <span class="text-xs text-gray-400">{{ $tournament->participants()->count() }}/{{ $tournament->max_slots }} slot</span>
            </div>

            {{-- Game Profile Info --}}
            @if($this->gameProfile)
                @php $gameName = \App\Models\UserGameProfile::getGameName($tournament->game_type); @endphp
                <div class="mb-6 bg-cyan-950/30 border border-cyan-500/30 rounded-xl p-4"
                     x-data="gameProfileCopy()">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 rounded-lg bg-cyan-500/20 flex items-center justify-center text-lg font-bold text-cyan-400">
                            {{ strtoupper(substr($tournament->game_type, 0, 2)) }}
                        </div>
                        <div>
                        ...
                        copyToClipboard(text, key) {
                            ...
                            if (navigator.clipboard && window.isSecureContext) {
                                navigator.clipboard.writeText(text).then(function() {
                                    self.copied = key;
                                    setTimeout(function() { self.copied = null; }, 2000);
                                }).catch(function() {
                ...
                <div class="mb-6 bg-yellow-950/30 border border-yellow-500/30 rounded-xl p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1">Tambahkan profil game di <a href="{{ route('profile') }}" class="text-cyan-400 hover:underline cursor-pointer">halaman profil</a> untuk mempermudah pendaftaran.</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($this->totalSteps > 1)
                <div class="mb-6">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-xs text-gray-500">Bagian {{ $currentStep + 1 }} dari {{ $this->totalSteps }}</span>
                        <span class="text-xs text-cyan-400 font-medium">{{ $currentSection['title'] ?? '' }}</span>
                    </div>
                    <div class="flex gap-1">
                        @for($i = 0; $i < $this->totalSteps; $i++)
                            <div class="flex-1 h-1 rounded-full transition-all {{ $i <= $currentStep ? 'bg-cyan-500' : 'bg-gray-700' }}"></div>
                        @endfor
                    </div>
                </div>
            @endif
            ...
            {{-- Registration Form --}}
            <form wire:submit="{{ $currentStep === $this->totalSteps - 1 ? 'submit' : 'nextStep' }}" class="space-y-4">
                @if(session('error'))
                ...
                @if($this->totalSteps > 1 && $currentSection)
                    <div class="bg-gradient-to-r from-cyan-900/20 to-blue-900/20 border border-cyan-500/30 rounded-xl p-4 mb-4">
                ...
                @if($currentSection['includesFixedFields'] ?? false)
                    <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-white mb-1">Nama Tim <span class="text-red-500">*</span></label>
                            ...
                            @error('team_name') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-white mb-1">Nama Kapten <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="captain_name" placeholder="Nama lengkap kapten tim"
                                   class="w-full bg-black/50 border border-gray-700 rounded p-3 text-white text-sm outline-none focus:border-cyan-500 transition">
                            @error('captain_name') <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-white mb-1">WhatsApp Kapten <span class="text-red-500">*</span></label>
                    ...
                    <div wire:key="field-{{ $field['id'] }}">
                        @switch($field['type'])
                            ...
                            @case('note')
                                <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6">
                                    ...
                                    @if($field['description'] ?? false)
                                        <p class="text-sm text-gray-400 whitespace-pre-wrap mt-2">{{ $field['description'] }}</p>
                                    @endif
                                </div>
                                @break
                                ...
                                <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-2 text-center">
                                    ...
                                    @if($field['description'] ?? false)
                                        @php
                                            ...
                                            $imageSrc = Str::startsWith($imagePath, ['http://', 'https://']) ? $imagePath : Storage::url($imagePath);
                                        @endphp
                                        <div class="relative group" x-data="{ showFullscreen: false }">
                                            ...
                                            <div class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                                                <button type="button" @click="showFullscreen = true" class="p-2 bg-black/70 rounded-lg text-white hover:bg-black transition cursor-pointer" title="Fullscreen">
                                                ...
                                                <div x-show="showFullscreen" x-cloak
                                                    ...
                                                    <div class="absolute top-4 left-1/2 -translate-x-1/2 flex gap-2 bg-black/80 backdrop-blur-sm px-3 py-2 rounded-full border border-gray-700">
                                                        <button type="button" @click.stop="
                                                            ...
                                                            w.document.close();
                                                        " class="p-2 text-white hover:text-cyan-400 transition cursor-pointer" title="Buka di tab baru">
                            ...
                            @case('link')
                                <a href="{{ $field['description'] ?? '#' }}" target="_blank" rel="noopener"
                                   class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6 flex items-center gap-4 hover:border-cyan-500 cursor-pointer block transition">
                                    ...
                                    <span class="font-bold text-cyan-400">{{ $field['label'] ?: $field['description'] }}</span>
                                </a>
                                @break
                            ...
                            @default
                                ...
                                <div class="bg-[#1a1a1a] border {{ $hasError ? 'border-red-500' : 'border-gray-800' }} rounded-xl p-6 transition focus-within:border-cyan-500">
                                    ...
                                    @if($field['imageUrl'] ?? false)
                                        @php
                                            ...
                                            $embedSrc = Str::startsWith($embedPath, ['http://', 'https://']) ? $embedPath : Storage::url($embedPath);
                                        @endphp
                                    ...
                                    {{-- Description --}}
                                    @if($field['description'] ?? false)
                                        <p class="text-xs text-gray-500 mb-3 whitespace-pre-wrap">{{ $field['description'] }}</p>
                                    @endif
                ...
                <div class="flex justify-between items-center mt-8 gap-4">
                    ...
                    @if($currentStep < $this->totalSteps - 1)
                        <button type="submit"
                            ...
                            <svg class="w-4 h-4" wire:loading.remove fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </button>
                    @else
                        <button type="submit" 
            ...
            <div class="text-center mt-12 text-[10px] text-gray-600">&copy; {{ date('Y') }} MIRAI Hub. Powered by MIRAI Indonesia.</div>
        @endif
    @endif
</div>
...
function registrationForm() {
    return {
        ...
        loadSavedData() {
            ...
            this.$watch('$wire.answers', () => this.saveData(), { deep: true });
        },
