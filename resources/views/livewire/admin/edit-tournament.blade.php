    ...
    <form wire:submit.prevent="save" class="flex-1 space-y-6 w-full">
        ...
        <div class="bg-[#1a1a1a] p-6 rounded-xl border-t-8 border-cyan-500 shadow-xl space-y-4">
            <div class="flex items-center justify-between gap-4 mb-2">
                <div class="flex items-center gap-4">
                    <a href="{{ route('admin.tournaments.index') }}"
                        class="p-2 text-gray-500 hover:text-white transition cursor-pointer">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t border-gray-800">
                <!-- GAME (Autocomplete) -->
                <div class="space-y-1" x-data="gameAutocomplete(@js($this->gameTypes), '{{ $game_type }}')" @click.outside="showDropdown = false">
                    ...
                    <div class="relative">
                        ...
                        <div x-show="showDropdown && search.length >= 2 && filteredGames.length > 0"
                            ...
                            class="absolute z-30 mt-1 w-full bg-[#0a0a0a] border border-cyan-500/30 rounded-xl shadow-2xl shadow-black/50 max-h-60 overflow-auto">
                        ...
                        <!-- No Results -->
                        <div x-show="showDropdown && search.length >= 2 && filteredGames.length === 0"
                            class="absolute z-30 mt-1 w-full bg-[#0a0a0a] border border-cyan-500/30 rounded-xl shadow-2xl shadow-black/50 p-4 text-center text-gray-500 text-sm">
                ...
                <div class="space-y-1">
                    ...
                    <input type="datetime-local" wire:model="event_date" required
                        style="color: #22d3ee !important; color-scheme: dark;"
                        class="w-full bg-gradient-to-br from-[#0c0c0f] to-[#0a0a0a] border border-gray-700/80 rounded-xl px-4 py-3 text-sm text-cyan-400 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/40 transition shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] [&::-webkit-calendar-picker-indicator]:invert [&::-webkit-calendar-picker-indicator]:brightness-[0.7] [&::-webkit-calendar-picker-indicator]:sepia [&::-webkit-calendar-picker-indicator]:saturate-[5] [&::-webkit-calendar-picker-indicator]:hue-rotate-[160deg] [&::-webkit-calendar-picker-indicator]:cursor-pointer">
                </div>

                <!-- BIAYA -->
                <div class="space-y-1">
                    <label class="text-[10px] uppercase text-gray-500 font-bold">Biaya</label>
                    <input type="number" wire:model="fee" min="0" required placeholder="0"
                        style="color: #22d3ee !important;"
                        class="w-full bg-gradient-to-br from-[#0c0c0f] to-[#0a0a0a] border border-gray-700/80 rounded-xl px-4 py-3 text-sm text-cyan-400 placeholder-gray-600 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/40 transition shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] hover:border-gray-600">
                </div>

                <!-- SLOT -->
                <div class="space-y-1">
                    <label class="text-[10px] uppercase text-gray-500 font-bold">Slot</label>
                    <input type="number" wire:model="max_slots" min="2" max="256" required placeholder="32"
                        style="color: #22d3ee !important;"
                        class="w-full bg-gradient-to-br from-[#0c0c0f] to-[#0a0a0a] border border-gray-700/80 rounded-xl px-4 py-3 text-sm text-cyan-400 placeholder-gray-600 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/40 transition shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] hover:border-gray-600">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                <div class="space-y-1">
                    ...
                    <select wire:model="status" style="color: #22d3ee !important; color-scheme: dark;"
                        class="w-full bg-gradient-to-br from-[#0c0c0f] to-[#0a0a0a] border border-gray-700/80 rounded-xl px-4 py-3 text-sm text-cyan-400 cursor-pointer focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/40 transition shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] hover:border-gray-600 [&>option]:bg-gray-900 [&>option]:text-cyan-400 [&>option]:py-2">
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            <div class="relative inline-block group" x-data="bannerPreview('{{ $currentBanner }}')">
                                <img src="{{ $currentBanner }}" alt="Banner"
                                    class="h-12 rounded border border-gray-700 cursor-pointer"
                    ...
                    @if ($banner)
                        <div class="mt-2 flex items-center gap-2" wire:loading.remove wire:target="banner">
                            ...
                            <div class="relative inline-block group" x-data="bannerPreview('{{ $newBanner }}')">
                                <img src="{{ $newBanner }}"
                                    class="h-16 rounded border border-cyan-500 object-cover cursor-pointer"
                                    alt="New Banner" @click="showFullscreen = true">
                                <div
                                    class="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                                    <button type="button" @click.stop="showFullscreen = true"
                                        class="p-1 bg-black/70 rounded text-white hover:text-cyan-400 cursor-pointer"
        ...
        <div class="flex items-center gap-4 pt-4 border-t border-gray-800">
            <a href="{{ route('admin.tournaments.index') }}"
                class="px-6 py-3 border border-gray-700 rounded-lg text-gray-300 hover:text-white hover:border-gray-500 transition cursor-pointer">Batal</a>
            <button type="submit"
