<div class="min-h-screen py-12 px-4">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6 mb-6">
            ...
            <form wire:submit="updateProfile" class="space-y-5">
                    ...
                    <div class="relative">
                        <div class="w-24 h-24 rounded-full bg-[#222] border-2 border-gray-700 overflow-hidden flex items-center justify-center">
                            ...
                            @elseif($this->avatarUrl)
                                <img src="{{ $this->avatarUrl }}" class="w-full h-full object-cover">
                            @else
                ...
                @error('newAvatar') <p class="text-red-400 text-sm">{{ $message }}</p> @enderror
        ...
        <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6 mb-6">
            <div class="flex items-center justify-between mb-6">
                ...
                @if($this->availableGames->count() > 0)
                    <button wire:click="openGameForm" class="text-sm text-cyan-400 hover:text-cyan-300 transition flex items-center gap-1 cursor-pointer">
                    ...
                    @foreach($this->gameProfiles as $profile)
                        ...
                        <div class="bg-[#111] border border-gray-800 rounded-lg p-4 flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-lg bg-[#222] flex items-center justify-center text-xl font-bold text-cyan-400">
                            ...
                            <div class="flex items-center gap-1">
                                <button wire:click="openGameForm('{{ $profile->game_type }}')" class="p-2 text-gray-400 hover:text-cyan-400 transition cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                    </svg>
                                </button>
                                <button wire:click="deleteGameProfile({{ $profile->id }})" wire:confirm="Hapus profil game ini?" class="p-2 text-gray-400 hover:text-red-400 transition cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-sm text-gray-600 mb-4">Tambahkan profil game untuk mempermudah pendaftaran turnamen</p>
                    <button wire:click="openGameForm" class="text-cyan-400 hover:text-cyan-300 text-sm font-medium cursor-pointer">
                        + Tambah Profil Game
                    </button>
                </div>
            @endif
        ...
        <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6">
            ...
            @if(!auth()->user()->password)
                <p class="text-gray-500 text-sm mb-4">Kamu login dengan Google. Set password untuk bisa login dengan email juga.</p>
            @endif
    ...
    <!-- Game Profile Modal -->
    @if($showGameForm)
    <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" wire:click.self="closeGameForm">
        <div class="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6 w-full max-w-md">
            <div class="flex items-center justify-between mb-6">
                ...
                <button wire:click="closeGameForm" class="text-gray-500 hover:text-white transition cursor-pointer">
            ...
            <form wire:submit="saveGameProfile" class="space-y-4">
                ...
                <div>
                    ...
                    @endif
                    @error('gameForm.game_type') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" wire:model="gameForm.username" placeholder="Username in-game kamu"
                           class="w-full px-4 py-3 bg-[#222] border border-gray-700 rounded-lg focus:outline-none focus:border-cyan-500 transition">
                    @error('gameForm.username') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" wire:model="gameForm.game_id" 
                           ...
                           class="w-full px-4 py-3 bg-[#222] border border-gray-700 rounded-lg focus:outline-none focus:border-cyan-500 transition">
                    @error('gameForm.game_id') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
